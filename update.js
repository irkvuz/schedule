...
let api = {
    ...
    getGroups: async (facultyId) => JSON.parse((await axios.get(`/?mode=1&id=${facultyId}`)).data.substr(1)),
    getTrimester: async () => JSON.parse((await axios.get(`/?mode=2`)).data.substr(1)),
    getSchedule: async (groupId, trimesterId) => JSON.parse((await axios.get(`/?mode=3&id=${groupId}&idt=${trimesterId}`)).data.substr(1)),
};
...
(async () => {
    try {
        ...
        for (let f of faculties) {
            ...
            let bar = new ProgressBar(`${newFaculty.name} [:bar] :current/:total :percent :etas`, {
                width: 20,
                total: groups.length,
            });
